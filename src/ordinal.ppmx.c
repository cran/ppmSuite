...
static void ordinal_ppmx(
                  int *y, double *co, int *nobs,
                  double *Xcon, int *Xcat, int *ncon, int *ncat, int *Cvec,
                  double *Xconp, int *Xcatp, int *npred, int *nordcat,
                  ...
                  int *Si, int *nclus, double *mu, double *sig2, double *zi, 
                  double *mu0, double *sig20, double *beta,
                  double *like, double *waic, double *lpml,
                  double *ispred, int *isordpred, double *ppred, int *predclass,
                  int *ordppred, double *rbpred, int *rbordpred,
  ...
  double *_sig2h = R_VectorInit(*nobs,0.0);;
  ...
  for(j = 0; j < *nobs; j++){
    _zi[j] = r_truncnorm(co[y[j]], co[y[j]+1],0, 1);
...
SEXP ORDINAL_PPMX(SEXP y, SEXP co, SEXP nobs, SEXP Xcon, SEXP Xcat, SEXP ncon, 
                        SEXP ncat, SEXP Cvec, SEXP Xconp, SEXP Xcatp, SEXP npred,
                        SEXP nordcat, SEXP meanModel, SEXP modelPriors, SEXP mh,
  ...
  ordinal_ppmx(INTEGER(y), REAL(co), &_nobs, REAL(Xcon), INTEGER(Xcat), &_ncon, &_ncat, 
                     INTEGER(Cvec), REAL(Xconp), INTEGER(Xcatp), &_npred, &_nordcat, 
                     ...
                     Siout, NCLUSout, MUout, SIG2out, Ziout, MU0out, SIG20out, BETAout,
                     LIKEout, WAICout, LPMLout, ISPREDout, ISORDPREDout, PPREDout, PREDCLASSout,
                     ORDPPREDout, RBPREDout, ORDRBPREDout, PREDCLASS_PROBout);
